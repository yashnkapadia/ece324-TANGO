...
def generate_bar_chart(out_path: Path) -> None:
    ...
    data = {"mappo": ([], []), "max_pressure": ([], []), "fixed_time": ([], []),
            "nema": ([], [])}
    ...
    for (ctrl, label, color, alpha), offset in zip(controllers, offsets):
        ...
        ax.bar(x + offset, [v / 1000 for v in means], width,
               yerr=[s / 1000 for s in stds], capsize=2,
    ...
    for i, (m, mp) in enumerate(zip(mappo_means, mp_means)):
        pct = (1 - m / mp) * 100
        ax.text(x[i] + offsets[0], m / 1000 + max(mappo_stds[i], 1) / 1000 + 5,
...
def generate_training_curve(out_path: Path) -> None:
    ...
    if warmstart_path.exists():
        ...
        ax.scatter(df_ws["episode"], df_ws["mean_global_reward"],
                   color="#aaaaaa", alpha=0.15, s=4, zorder=0)
        smoothed_ws = df_ws["mean_global_reward"].rolling(window, min_periods=1).mean()
        ax.plot(df_ws["episode"].values, smoothed_ws.values,
                color="#888888", linewidth=1.5, alpha=0.7,
                label="Warm-start (AM only)", linestyle="--", zorder=1)

    # Boundary
    ax.axvline(x=200, color="#cccccc", linewidth=1, linestyle=":", zorder=3)
    ax.text(205, ax.get_ylim()[0] if ax.get_ylim()[0] != 0 else -1.5,
            "Curriculum\nstarts", color="#666666", fontsize=7, va="bottom")
    ...
    for scenario_id, color in SCENARIO_COLORS.items():
        ...
        ax.plot(x_vals, smoothed.values, color=color, linewidth=1.8, alpha=0.9,
                label=SCENARIO_LABELS[scenario_id], zorder=2)

    # Best checkpoint
    best_total = 432 + 200
    ax.axvline(x=best_total, color="#d6604d", linewidth=1, linestyle="--", alpha=0.7, zorder=3)
    ax.text(best_total + 5, ax.get_ylim()[1] * 0.95, "Best (ep 632)",
            color="#d6604d", fontsize=8, fontweight="bold", va="top")
...
def generate_gate_fraction(out_path: Path) -> None:
    ...
    if warmstart_path.exists():
        ...
        if "gate_fraction" in df_ws.columns:
            ...
            ax.plot(df_ws["episode"].values, sm_ws.values,
                    color="#888888", linewidth=1.5, alpha=0.7, linestyle="--", zorder=1)
    ...
    ax.scatter(x_vals, df_sorted["gate_fraction"] * 100,
               color=C_MAPPO, alpha=0.15, s=4, zorder=1)
...
def generate_architecture_diagram(out_path: Path) -> None:
    ...
    def _box(x, y, w, h, fc, ec, lw=1.2):
        p = FancyBboxPatch((x - w/2, y - h/2), w, h,
                           boxstyle="round,pad=0.3", facecolor=fc, edgecolor=ec,
    ...
    # ── Track labels ──
    ax.text(50, 54, "ACTOR  (per-agent, decentralized)", ha="center", fontsize=8,
            fontweight="bold", color="#555555", family="monospace")

    # ── Input boxes ──
    _box(10, 42, 16, 9, C_INPUT, C_INPUT_E)
    ax.text(10, 44, "Local Obs", ha="center", va="center", fontsize=8,
            fontweight="bold", color=C_LABEL)
    ax.text(10, 40.5, "queue, density,\nphase, min-green", ha="center", va="center",
            fontsize=6.5, color="#555555")

    _box(10, 30, 16, 6.5, C_PROC, C_PROC_E)
    ax.text(10, 31.5, "MP Action", ha="center", va="center", fontsize=8,
            fontweight="bold", color=C_LABEL)
    ax.text(10, 28.5, "one-hot  |  6 dim", ha="center", va="center",
            fontsize=6.5, color="#555555")
    ...
    ax.text(28, 36.5, "Concat", ha="center", va="center", fontsize=7,
            fontweight="bold", color=C_LABEL)

    # ── Welford ──
    _arrow(32, 36.5, 37, 36.5)
    _box(42, 36.5, 10, 5.5, C_PROC, C_PROC_E)
    ax.text(42, 38, "Welford", ha="center", va="center", fontsize=7,
            fontweight="bold", color=C_LABEL)
    ax.text(42, 35.5, "$\\mu / \\sigma$", ha="center", va="center",
            fontsize=7, color="#555555")

    # ── Shared MLP ──
    _arrow(47, 36.5, 52, 36.5)
    _box(60, 36.5, 14, 8, C_MLP, C_MLP_E, lw=1.8)
    ax.text(60, 39, "Shared MLP", ha="center", va="center", fontsize=8,
            fontweight="bold", color=C_LABEL)
    ax.text(60, 36.5, "128→ReLU→128", ha="center", va="center",
            fontsize=7, color="#555555")
    ax.text(60, 34, "×12 agents", ha="center", va="center",
            fontsize=6.5, color="#888888", fontstyle="italic")

    # ── Gate Head ──
    _arrow(67, 39.5, 74, 46)
    _box(80, 46, 13, 7, C_GATE, C_GATE_E, lw=1.5)
    ax.text(80, 48, "Gate Head", ha="center", va="center", fontsize=7.5,
            fontweight="bold", color=C_LABEL)
    ax.text(80, 45.5, "follow / override", ha="center", va="center",
            fontsize=6.5, color="#555555")
    ax.text(80, 43.5, "init: 98% follow", ha="center", va="center",
            fontsize=6, color="#888888", fontstyle="italic")

    # ── Phase Head ──
    _arrow(67, 34, 74, 28)
    _box(80, 28, 13, 7, C_PHASE, C_PHASE_E, lw=1.5)
    ax.text(80, 30, "Phase Head", ha="center", va="center", fontsize=7.5,
            fontweight="bold", color=C_LABEL)
    ax.text(80, 27.5, "6 phases (masked)", ha="center", va="center",
            fontsize=6.5, color="#555555")
    ax.text(80, 25.5, "softmax over valid", ha="center", va="center",
            fontsize=6, color="#888888", fontstyle="italic")
    ...
    ax.text(96, 39.5, "Action", ha="center", va="center", fontsize=7.5,
            fontweight="bold", color=C_LABEL)
    ax.text(96, 37, "g=0→MP", ha="center", va="center",
            fontsize=6, color=C_GATE_E, fontweight="bold")
    ax.text(96, 35, "g=1→$\\pi$", ha="center", va="center",
            fontsize=6, color=C_PHASE_E, fontweight="bold")

    # ── CTDE separator ──
    ax.plot([0, 100], [20, 20], color="#cccccc", linewidth=1, linestyle="--", zorder=1)
    ax.text(50, 21, "CTDE boundary — critic used during training only",
            ha="center", fontsize=6.5, color="#999999", fontstyle="italic")

    # ── Critic track (bottom) ──
    ax.text(50, 17.5, "CRITIC  (centralized, training only)", ha="center", fontsize=8,
            fontweight="bold", color="#555555", family="monospace")

    _box(15, 10, 18, 6, C_CRITIC, C_CRITIC_E)
    ax.text(15, 11.5, "Global Obs", ha="center", va="center", fontsize=8,
            fontweight="bold", color=C_LABEL)
    ax.text(15, 9, "all 12 agents  |  ~460d", ha="center", va="center",
            fontsize=6.5, color="#555555")

    _arrow(24, 10, 35, 10)

    _box(48, 10, 22, 6, C_CRITIC, C_CRITIC_E, lw=1.5)
    ax.text(48, 11.5, "Centralized Critic", ha="center", va="center", fontsize=8,
            fontweight="bold", color=C_LABEL)
    ax.text(48, 9, "128→ReLU→128→ReLU→1", ha="center", va="center",
            fontsize=6.5, color="#555555")

    _arrow(59, 10, 68, 10)

    _box(74, 10, 10, 5.5, C_PROC, C_CRITIC_E)
    ax.text(74, 11, "V(s)", ha="center", va="center", fontsize=8,
            fontweight="bold", color=C_LABEL)
    ax.text(74, 8.5, "scalar value", ha="center", va="center",
