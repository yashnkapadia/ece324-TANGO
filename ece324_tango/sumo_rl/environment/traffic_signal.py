...
class TrafficSignal:
    ...
    def _build_phases(self):
        ...
        # Identify green phase indices in the NATIVE program (do NOT override it).
        # A green phase is any phase containing 'G' or 'g' that is not a yellow
        # or all-red phase.  We keep the native program (with its yellow, all-red,
        # and coordinated transitions) running in SUMO.
        self.green_phases = []
        self._green_phase_indices = []  # index into native program
        for idx, phase in enumerate(phases):
            state = phase.state
            if "y" not in state and (state.count("r") + state.count("s") != len(state)):
                self.green_phases.append(phase)
                self._green_phase_indices.append(idx)
        self.num_green_phases = len(self.green_phases)
        # all_phases kept for observation compatibility (phase_one_hot size)
        self.all_phases = self.green_phases
    ...
    def update(self):
        """Track timing.  SUMO handles yellow/all-red natively."""
        self.time_since_last_phase_change += 1

    def set_next_phase(self, new_phase: int):
        """Request a green phase from the native TLS program.

        Uses traci.trafficlight.setPhase() so SUMO transitions through
        yellow and all-red clearance phases defined in the network.
        ...
        if (
            ...
            # Switch to the target green phase in the native program.
            # SUMO will transition through yellow/all-red automatically.
            native_idx = self._green_phase_indices[new_phase]
            self.sumo.trafficlight.setPhase(self.id, native_idx)
            self.green_phase = new_phase
            self.next_action_time = self.env.sim_step + self.delta_time
            self.time_since_last_phase_change = 0
