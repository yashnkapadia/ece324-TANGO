...
PIRA is a message-passing graph neural network intended to predict traffic performance metrics (person-time-loss, throughput, average trip time) directly from scenario descriptors, without running a full SUMO simulation. The architecture represents the road network as a graph where intersections are nodes and road segments are edges, with node features derived from signal timing parameters and demand descriptors. The model, training loop, scenario predictor, and a synthetic dummy-data generator are all implemented in \texttt{ece324\_tango/pira/}. On synthetic data, the network trains and produces predictions end-to-end. However, training on the real ASCE-generated baseline dataset (produced via \texttt{scripts/generate\_baseline\_dataset.py}) converged poorly within the project timeline due to feature-engineering issues mapping ASCE rollouts into PIRA's expected graph input format. PIRA's accuracy and inference-time targets (MAPE~$\leq$~10\%, $<$5\,s) are therefore not evaluated; completing dataset integration is the primary remaining work item.
