...
Starting from the warm-start checkpoint, the policy trains on all four scenarios from Section~\ref{sec:scenarios} in round-robin order, using 8 parallel SUMO workers via Python multiprocessing with \texttt{libsumo}. Learning rates are scaled by $1/\sqrt{8}$ to compensate for the larger effective batch size from parallel collection~\cite{schulman_ppo}, yielding actor $1.06 \times 10^{-4}$ and critic $3.54 \times 10^{-4}$. Episodes run for 900\,s (AM/PM Peak) or 1200\,s (Demand Surge, Midday Multimodal) at 5\,s decision intervals.
...
Although incomplete, some progress was made on the PIRA layer. The general pipeline was established under \texttt{ece324\_tango/pira/}: \texttt{pira.py} takes a Parquet rollout file from ASCE together with the OSM file used for SUMO, converts the road network into a graph neural network over intersections (nodes) and road segments (edges), and exports a trained model that the companion \texttt{planner.py} consumes for road-infrastructure scenarios. Dataset preparation is handled by \texttt{scripts/generate\_baseline\_dataset.py} and \texttt{scripts/generate\_asce\_dataset.py}. However, the model and the planner were trained on synthetic data only, to show that the pipeline runs end-to-end; the planner used in the presentation GIF was likewise running on synthetic data, and served only as a communication aid for the audience. PIRA's accuracy and inference-time targets (MAPE~$\leq$~10\%, $<$5\,s) are therefore not evaluated, and completing real-data integration is the primary remaining work item.


\section{Future Work and Potential Impacts}
\label{sec:future}

The two outstanding items relative to the proposal are completing PIRA and validating ASCE against real traffic: while real TMC statistics drive demand, much of the policy was trained against SUMO output as ground truth, and closing that loop is not realistic without an instrumented deployment. ASCE has the potential to reduce total intersection delay and the productivity losses that come with it; conversely, faster vehicle traffic could induce additional car demand and lower the perceived need for public transit, so any deployment should be paired with a multimodal monitoring plan.


\section{Reproducibility}
\label{sec:reproducibility}

All code, trained models, and evaluation scripts are available at \url{https://github.com/yashnkapadia/ece324-TANGO}. Reproducing the headline results is \texttt{pixi install} followed by \texttt{pixi run eval-matrix}; the headline checkpoint is \texttt{models/asce\_mappo\_curriculum\_best.pt} and the four curriculum demand scenarios live under \texttt{sumo/demand/curriculum/}. The full warm-start + curriculum pipeline takes ${\sim}$8~h on an NVIDIA RTX~4070 Laptop; total compute for all reported experiments is ${\sim}$40 GPU-hours.
