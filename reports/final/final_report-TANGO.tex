...
\begin{abstract}
This project aimed to build a two-model system to optimize traffic by reducing the total amount of time spent waiting at intersections, and help predict traffic change as irregular conditions such as lane closure or opening of new transit lines. Only the first layer (ASCE), which reduces the time spent waiting at intersections, could be completed within the timeframe, and the transit planner layer (PIRA) could not be finished. ASCE has been proven to function and outperform the Max-Pressure algorithm under all four scenarios tested, with the largest gains on out-of-distribution conditions. Across 5 random seeds, the reductions in person-time-loss compared to Max-Pressure are as follows: AM Peak: $-10.1\%$; PM Peak: $-6.3\%$; Demand Surge: $-11.0\%$; Midday Multimodal: $-13.8\%$. ASCE additionally outperforms two further baselines on every scenario---SUMO's auto-generated Actuated Default signal programs and a naive Fixed-Time controller---while improving vehicle delay fairness (Jain index) by ${\sim}30\%$ relative to Max-Pressure. The project code and trained models are publicly available.\footnote{\url{https://github.com/yashnkapadia/ece324-TANGO}}
\end{abstract}
...
\begin{figure}[t]
  \centering
  \resizebox{\columnwidth}{!}{\input{asce_architecture.tex}}
...
Starting from the warm-start checkpoint, the policy trains on all four scenarios in round-robin order (Table~\ref{tab:scenarios}), using 8 parallel SUMO workers via Python multiprocessing with \texttt{libsumo}. Learning rates are scaled by $1/\sqrt{8}$ to compensate for the larger effective batch size from parallel collection~\cite{schulman_ppo}, yielding actor $1.06 \times 10^{-4}$ and critic $3.54 \times 10^{-4}$. Episodes run for 900\,s (AM/PM Peak) or 1200\,s (Demand Surge, Midday Multimodal) at 5\,s decision intervals.
...
\begin{table}[t]
  \centering
  \small
  \caption{Person-time-loss (seconds, mean $\pm$ std over 5 seeds) across four evaluation scenarios. Three baselines are compared: Max-Pressure (reactive, queue-differential), Actuated Default (SUMO's auto-generated actuated programs from OSM), and Fixed-Time (uniform 30\,s green cycle). MAPPO/MP values below 0.90 meet the proposal target (bold).}
  \label{tab:results}
  \begin{tabular}{@{}llrrc@{}}
    \toprule
    \textbf{Scenario} & \textbf{Controller} & \textbf{PTL (s)} & \textbf{MAPPO/MP} & \textbf{$\geq$10\%?} \\
    \midrule
    \multirow{4}{*}{AM Peak}
      & Max-Pressure     & $427{,}943 \pm 12{,}390$ & --- & \\
      & Actuated Default & $439{,}932 \pm 15{,}442$ & --- & \\
      & Fixed-Time       & $456{,}689 \pm 16{,}047$ & --- & \\
      & MAPPO (best)     & $384{,}598 \pm \phantom{0}5{,}946$ & \textbf{0.899} & \checkmark \\
    \midrule
    \multirow{4}{*}{PM Peak}
      & Actuated Default & $490{,}484 \pm \phantom{0}9{,}981$ & --- & \\
      & Fixed-Time       & $492{,}550 \pm 15{,}828$ & --- & \\
      & Max-Pressure     & $518{,}755 \pm 15{,}072$ & --- & \\
      & MAPPO (best)     & $486{,}226 \pm 28{,}139$ & 0.937 & \\
    \midrule
    \multirow{4}{*}{Demand Surge}
      ...
      & MAPPO (best)     & $447{,}626 \pm \phantom{0}7{,}284$ & \textbf{0.890} & \checkmark \\
    \midrule
    \multirow{4}{*}{Midday Mult.}
      ...
      & MAPPO (best)     & $469{,}303 \pm 16{,}586$ & \textbf{0.862} & \checkmark \\
    \bottomrule
  \end{tabular}
\end{table}

The best curriculum checkpoint meets the 10\% improvement target on AM Peak, Demand Surge, and Midday Multimodal traffic. PM Peak, the scenario with the highest total volume and a tidal reversal pattern, shows a 6.3\% improvement over Max-Pressure. We hypothesize that the relative under-performance of MAPPO in PM Peak is due to the complexity of managing high-volume, reversed flow traffic conditions and expect it to be ameliorated with further training. The relative ordering of baselines shifts across scenarios: Max-Pressure is the strongest baseline on AM Peak and Midday Multimodal, while Actuated Default is strongest on PM Peak and Demand Surge---the two high-volume arterial-dominant scenarios where its built-in phase extension provides an advantage similar to Max-Pressure's queue responsiveness. MAPPO beats all baselines on every scenario regardless of which baseline is strongest.
...
The gate fraction (proportion of time steps where the agent overrides Max-Pressure) starts near 2\% due to the initialization bias and rises to approximately 30\% by the end of curriculum training (Figure~\ref{fig:gate_fraction}). This confirms that the policy learns to selectively override rather than replacing Max-Pressure wholesale. The override rate is highest during demand transitions (surge onset, tidal reversal) and lowest during stable steady-state flow---consistent with the hypothesis that Max-Pressure is near-optimal under stationary demand but suboptimal during transients.
...
The training curve does not converge smoothly; round-robin curriculum causes oscillation as scenario-specific gradients conflict. The best-model checkpoint mitigates but does not solve this. More principled curriculum strategies (e.g., prioritized experience replay across scenarios, or multi-task loss weighting) may improve stability. PM~Peak remains below the 10\% target, suggesting that the current curriculum does not allocate sufficient training to the highest-volume scenario. The Fixed-Time baseline uses a uniform 30\,s green cycle rather than the actual Toronto signal timing programs; a more realistic comparison would use the real fixed-time plans from the City. Additionally, legally mandated safety constraints (minimum green, maximum green, pedestrian recall) are not enforced on MAPPO's phase selections, which would be required for any real-world deployment.

\subsection{PIRA: Scenario Planning Surrogate (Incomplete)}
\label{sec:pira}

Although incomplete, some progress was made on the PIRA layer. The general pipeline was established, having a Python file (\texttt{pira.py}) that takes a Parquet rollout file from ASCE as well as the OSM file used for SUMO, converts the road network into a graph neural network operating over intersections (nodes) and road segments (edges), and exports a trained model which can then be used in a separate Python file dedicated to planning road infrastructure changes (\texttt{planner.py}). Both files live under \texttt{ece324\_tango/pira/}, and dataset preparation is handled by \texttt{scripts/generate\_baseline\_dataset.py} (Max-Pressure and Fixed-Time rollouts) and the companion \texttt{scripts/generate\_asce\_dataset.py} (trained-policy rollouts), which together produce the Parquet files \texttt{pira.py} consumes. However, the model was not trained on real data---it was trained on synthetic data to show that the pipeline works as intended, and so was the planner. The planner used in the presentation GIF was also running on garbage data; it only served as a communication tool for the audience for them to understand what we wished to achieve. PIRA's accuracy and inference-time targets (MAPE~$\leq$~10\%, $<$5\,s) are therefore not evaluated, and completing real-data integration is the primary remaining work item.
...
All code, trained models, and evaluation scripts are available at \url{https://github.com/yashnkapadia/ece324-TANGO}. The repository includes:
\begin{itemize}[nosep]
  \item \texttt{pixi.toml} --- fully locked dependency specification (CUDA 12.6, PyTorch 2.7.1, SUMO 1.26.0)
  \item \texttt{pixi run train-asce-toronto-demand} --- reproduces the training pipeline
  \item \texttt{pixi run eval-asce-toronto-demand} --- reproduces evaluation
  \item \texttt{models/asce\_mappo\_curriculum\_best.pt} --- best curriculum checkpoint
  \item \texttt{sumo/demand/curriculum/} --- all four scenario demand files
\end{itemize}
Training on an NVIDIA RTX~4070 Laptop GPU takes approximately 8 hours (200 warm-start episodes + 600 curriculum episodes with 8 parallel workers). Total compute for all reported experiments, including preliminary and failed runs, is approximately 40 GPU-hours.
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{IPPO baseline replaced with action-gate ablation.} The proposal listed Independent PPO as a baseline to ablate the centralized critic. We instead focused on the action-gate residual architecture, which provides a more informative ablation of the Max-Pressure initialization benefit. The centralized critic ablation remains future work.
...
% TODO: Fill checklist (same structure as interim, updated for final results)
...
\item {\bf Claims}
    ...
    \item[] Justification: The abstract states specific MAPPO/MP improvement percentages that are directly supported by Table~\ref{tab:results}. The incomplete status of PIRA is explicitly noted.

\item {\bf Limitations}
    \item[] Question: Does the paper discuss the limitations of the work performed by the authors?
    \item[] Answer: \answerYes{}
    \item[] Justification: Section~\ref{sec:analysis} discusses PM Peak underperformance, curriculum instability, and remaining engineering limitations. Section~\ref{sec:scope} lists scope reductions.

\item {\bf Theory Assumptions and Proofs}
    \item[] Answer: \answerNA{}
    \item[] Justification: No theoretical results are claimed.

\item {\bf Experimental Result Reproducibility}
    \item[] Answer: \answerYes{}
    \item[] Justification: Full code, trained models, locked dependencies, and reproduction commands are provided (Section~7).

\item {\bf Open access to data and code}
    \item[] Answer: \answerYes{}
    \item[] Justification: GitHub repository linked in Section~7 with all code, models, and data.

\item {\bf Experimental Setting/Details}
    \item[] Answer: \answerYes{}
    \item[] Justification: Hyperparameters in Appendix~\ref{app:hyperparams}, training pipeline in Section~\ref{sec:training}, evaluation protocol in Section~\ref{sec:results}.

\item {\bf Experiment Statistical Significance}
    \item[] Answer: \answerYes{}
    \item[] Justification: All results report mean $\pm$ std over 5 random seeds.

\item {\bf Experiments Compute Resources}
    \item[] Answer: \answerYes{}
    \item[] Justification: GPU type, training time, and total compute budget reported in Section~7.

\item {\bf Code Of Ethics}
    \item[] Answer: \answerYes{}
    \item[] Justification: Research uses only publicly available traffic statistics and open-source simulation tools.

\item {\bf Broader Impacts}
    \item[] Answer: \answerYes{}
    \item[] Justification: Section~6 discusses both positive (delay reduction) and negative (induced demand) potential impacts.

\item {\bf Safeguards}
    \item[] Answer: \answerNA{}
    \item[] Justification: The project uses anonymized, aggregated traffic statistics with no privacy or misuse risk.

\item {\bf Licenses for existing assets}
    \item[] Answer: \answerYes{}
    \item[] Justification: SUMO (EPL-2.0), SUMO-RL (MIT), MAPPO reference implementation (MIT) are cited.

\item {\bf New Assets}
    \item[] Answer: \answerYes{}
    \item[] Justification: Trained models and vendored SUMO-RL modifications released under the project repository.
...
Claude Code (Anthropic, Claude Opus) was used as a development assistant throughout this project for: code implementation and debugging of the ASCE training pipeline, reward function iteration, parallel training infrastructure, and drafting portions of this report. All generated code was reviewed, tested, and modified by the authors. Claude Code was not used to generate experimental results or evaluation metrics---all reported numbers come from SUMO simulation runs.
