...
from ece324_tango.modeling import benchmark_backends, predict, train
...
def test_cli_reward_modes_include_time_loss():
    ...
    assert bench_default.default == "objective"
    assert "time_loss" in train._VALID_REWARD_MODES
    assert "time_loss" in predict._VALID_REWARD_MODES
    assert "time_loss" in benchmark_backends._VALID_REWARD_MODES
